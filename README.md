...
For an issue-driven route, `ralphworks init` also creates an optional [issue to draft PR workflow](docs/USAGE.md#issue-to-draft-pr-workflow). A maintainer's `ralphworks:run` label starts a checked run and opens a draft PR when it completes. This requires a repository check command and a separate token for PR delivery.
