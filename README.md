...
For issue and PR driven work, `ralphworks init` creates [scenario workflows](docs/USAGE.md#issue-to-draft-pr-workflow) for issue implementation, PRD splitting and serial implementation, dependency promotion, PR review and feedback, branch updates, and architecture proposals. Maintainer labels authorize model runs. Configure a repository check command and a separate token for GitHub delivery.
