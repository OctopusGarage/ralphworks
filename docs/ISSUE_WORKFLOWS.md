...
To use Matt Pocock's architecture skill as the survey input, set the repository variable `RALPHWORKS_ARCHITECTURE_SKILL_REF` to a commit SHA or a branch name such as `main`. A SHA keeps the skill fixed; a branch fetches its current commit at the start of each run. The workflow checks out `mattpocock/skills`, validates the architecture and `codebase-design` skill files, and installs that snapshot for Pi. It records the resolved commit in the run artifact and proposal Issue. All iterations in one run use that same snapshot. Remove the variable to use RalphWorks' built-in architecture prompt again.

The scheduled task adapts the skill's survey and visual report stages for unattended execution. It saves `.ralph/architecture.html` as an Actions artifact and puts a concise proposal in the Issue. The skill's interactive candidate selection, grilling, and implementation stages require a person and are not run by the schedule. A missing or incompatible skill source fails the run instead of silently falling back. The workflow only reads the target repository during review; maintainers decide whether to implement the resulting Issue. Review upstream changes before enabling a moving branch, since skill instructions can direct Pi's tools. Run `ralphworks init` to generate the workflow in a new repository; for an existing installation, update its generated `ralphworks-architecture.yml` from the current RalphWorks template without replacing local customizations.
...
If the agent needs a human decision, it stops with `needs_input`. Issue workflows put its question on the relevant Issue; PR review, feedback, and branch update workflows put it on the PR. They add `ralphworks:blocked` and do not deliver that run's proposed change. Review the question and run artifacts, update the Issue, sub-issue, or PR with the answer, then remove the status label and re-add the original trigger label. This starts a fresh run from the current repository state. The agent first investigates questions it can resolve itself; the human-input path is for decisions or information it cannot obtain.
