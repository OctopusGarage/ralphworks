...
Before submitting a pull request, run `pnpm verify:local` and describe the behavior changed, verification performed, and any limits. GitHub CI repeats the gate on Ubuntu and macOS. `main` requires a pull request, passing `verify`, `scan`, and `infrastructure` checks, and one approval from a reviewer with write access. Authors cannot approve their own pull requests. If no eligible reviewer is available, a repository administrator must explicitly adjust the review requirement for the merge and restore it afterward.
