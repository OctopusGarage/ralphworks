...
test("remote dispatch watches a run and downloads its result", async () => {
  ...
  const result = await runRemoteJob({
    repo: "OctopusGarage/mbti-lab", ref: "ralph-test", jobPath: "job.yaml", cwd,
    command: async (args) => {
      calls.push(args);
      if (args[0] === "workflow") return { exitCode: 0, stdout: "https://github.com/OctopusGarage/mbti-lab/actions/runs/123\n", stderr: "" };
...
test("remote preserves the downloaded terminal status when Actions exits nonzero", async () => {
  ...
  const result = await runRemoteJob({
    repo: "OctopusGarage/mbti-lab", ref: "ralph-test", jobPath: "job.yaml", cwd,
    command: async (args) => {
      if (args[0] === "workflow") return { exitCode: 0, stdout: "https://github.com/OctopusGarage/mbti-lab/actions/runs/456\n", stderr: "" };
      if (args[1] === "watch") return { exitCode: 1, stdout: "", stderr: "" };
      if (args[1] === "download") {
        await writeFile(join(cwd, ".ralph", "remote", "456", "result.json"), JSON.stringify({ status: "blocked", reason: "checks are required" }));
        return { exitCode: 0, stdout: "", stderr: "" };
      }
      throw new Error(`unexpected gh call: ${args.join(" ")}`);
    },
...
test("remote does not claim completion if a later Actions step fails", async () => {
  ...
  const result = await runRemoteJob({
    repo: "OctopusGarage/mbti-lab", ref: "ralph-test", jobPath: "job.yaml", cwd,
    command: async (args) => {
      if (args[0] === "workflow") return { exitCode: 0, stdout: "https://github.com/OctopusGarage/mbti-lab/actions/runs/789\n", stderr: "" };
      if (args[1] === "watch") return { exitCode: 1, stdout: "", stderr: "" };
      if (args[1] === "download") {
...
test("remote does not mislabel a task limit as an Actions infrastructure failure", async () => {
  ...
  const result = await runRemoteJob({
    repo: "OctopusGarage/mbti-lab", ref: "ralph-test", jobPath: "job.yaml", cwd,
    command: async (args) => {
      if (args[0] === "workflow") return { exitCode: 0, stdout: "https://github.com/OctopusGarage/mbti-lab/actions/runs/790\n", stderr: "" };
      if (args[1] === "watch") return { exitCode: 1, stdout: "", stderr: "" };
      if (args[1] === "download") {
