...
test("remote dispatch passes an explicit resume source", async () => {
  ...
  await runRemoteJob({
    ...
    command: async (args) => {
      if (args[0] === "workflow") {
        dispatch = args;
        return { exitCode: 0, stdout: "https://github.com/acme/widgets/actions/runs/456\n", stderr: "" };
      }
...
test("remote preserves the downloaded terminal status when Actions exits nonzero", async () => {
  ...
  const result = await runRemoteJob({
    ...
    cwd,
    command: async (args) => {
      if (args[0] === "workflow") return { exitCode: 0, stdout: "https://github.com/acme/widgets/actions/runs/456\n", stderr: "" };
      if (args[1] === "watch") return { exitCode: 1, stdout: "", stderr: "" };
      if (args[1] === "download") {
        await writeFile(
...
test("remote does not claim completion if a later Actions step fails", async () => {
  ...
  const result = await runRemoteJob({
    ...
    cwd,
    command: async (args) => {
      if (args[0] === "workflow") return { exitCode: 0, stdout: "https://github.com/acme/widgets/actions/runs/789\n", stderr: "" };
      if (args[1] === "watch") return { exitCode: 1, stdout: "", stderr: "" };
      if (args[1] === "download") {
...
test("remote does not mislabel a task limit as an Actions infrastructure failure", async () => {
  ...
  const result = await runRemoteJob({
    ...
    cwd,
    command: async (args) => {
      if (args[0] === "workflow") return { exitCode: 0, stdout: "https://github.com/acme/widgets/actions/runs/790\n", stderr: "" };
      if (args[1] === "watch") return { exitCode: 1, stdout: "", stderr: "" };
      if (args[1] === "download") {
