...
const job: RalphJob = { name: "worker", task: "test", progressFile: ".ralph/progress.md", maxIterations: 1, mode: "local", checkTimeoutSeconds: 60, commit: "none", checks: [] };
...
test("PiProcessRunner returns a worker result after the process exits", async () => {
  ...
  await writeFile(workerPath, 'process.on("message", () => process.send({result:{status:"continue",summary:"done"}}, () => process.exit(0)));');
  const result = await new PiProcessRunner({ workerPath }).runIteration({ job, iteration: 1, cwd, progress: "", signal: new AbortController().signal });
...
test("PiProcessRunner aborts the worker process group", async () => {
  ...
  const workerPath = join(cwd, "worker.cjs");
  await writeFile(workerPath, 'const {spawn}=require("node:child_process"); process.on("message", () => { spawn("sh", ["-c", "sleep 0.3; touch late-marker"], {stdio:"ignore"}); setInterval(()=>{},1000); });');
